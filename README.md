...
### 🚨 障害対応フロー (Incident Response)
アラート発報から復旧までのフローを以下のように定めています。今回は SNS メール通知を採用していますが、実務では PagerDuty や Slack 連携を想定しています。

1.  **Detection (検知)**: CloudWatch Alarm が SLO 違反（レイテンシ悪化 / 5xx エラー）を検知。
2.  **Notification (通知)**: SNS 経由で管理者にアラートメールを送信。
3.  **Triage (一次対応)**:
    * ログ (CloudWatch Logs Insights) からエラー原因を特定。
    * ECS タスクの再起動、または旧バージョンへのロールバックを実施。
4.  **Post-Mortem (事後分析)**:
